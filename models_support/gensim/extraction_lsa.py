...
PATH_TO_DICTIONARY                  : str   = '../exports/gensim/corpora_dictionary.bin'

PATH_TO_LSA_MODELS_DIRECTORY        : str   = '../exports/gensim/lsa_variations/'
PATH_TO_LSA_MODELS_TOPIC_COHERENCE  : str   = '../exports/gensim/lsa_topics_coherence.png'
PATH_TO_LSA_BEST_MODEL              : str   = '../exports/gensim/lsa_best_model.bin'
PATH_TO_LSA_BEST_MODEL_TOPICS       : str   = '../exports/gensim/lsa_best_model_topics.txt'
...
final_number_of_documents : int = load_corpus.get_number_of_documents(PATH_TO_DOCUMENTS, NUMBER_DOCUMENTS)
num_topics_max : int = min(final_number_of_documents, compute_max_number_of_topics_possible(dictionary, MAX_USABLE_RAM_GB))
num_topics_step : int = math.floor(num_topics_max / NUMBER_OF_TOPIC_VARIATIONS_TO_TEST)

model_list : List[Dict[str, Any]] = []
for num_topics in tqdm.trange(num_topics_step, num_topics_max + 1, num_topics_step, desc='🚀 Creating LSA models', leave=True):
    ...
    model_save_path : str = f'{PATH_TO_LSA_MODELS_DIRECTORY}lsa_model_{str(num_topics)}.bin'
...
plt.savefig(PATH_TO_LSA_MODELS_TOPIC_COHERENCE)

best_model_entry : Dict[str, Any] = max(model_list, key = lambda key: key['coherence_score'])
best_model : gensim.models.LsiModel = gensim.models.LsiModel.load(best_model_entry['model_path'])
best_model.save(PATH_TO_LSA_BEST_MODEL)

file = open(PATH_TO_LSA_BEST_MODEL_TOPICS, 'wt')
